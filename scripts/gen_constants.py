...
Usage:
    python3 scripts/gen_constants.py          # regenerate the header
...
def collect_all():
    ...
    array('atan_P', [mpf(s) for s in [
        '-4.283708356338736809269381409828726405572E1',
        '-8.636132499244548540964557273544599863825E1',
        '-5.713554848244551350855604111031839613216E1',
        '-1.371405711877433266573835355036413750118E1',
        '-8.638214309119210906997318946650189640184E-1',
    ]], 'atan P(t^2) numerator, 5 coefficients')
    array('atan_Q', [mpf(s) for s in [
        ' 1.285112506901621042780814422948906537959E2',
        ' 3.361907253914337187957855834229672347089E2',
        ' 3.180448303864130128268191635189365331680E2',
        ' 1.307244136980865800160844625025280344686E2',
    ...
    array('asin_pS', [mpf(s) for s in [
        '-8.358099012470680544198472400254596543711E2',
        ' 3.674973957689619490312782828051860366493E3',
        '-6.730729094812979665807581609853656623219E3',
        ' 6.643843795209060298375552684423454077633E3',
        '-3.817341990928606692235481812252049415993E3',
        ' 1.284635388402653715636722822195716476156E3',
        '-2.410736125231549204856567737329112037867E2',
        ' 2.219191969382402856557594215833622156220E1',
        '-7.249056260830627156600112195061001036533E-1',
        ' 1.055923570937755300061509030361395604448E-3',
    ]], 'asin P(x^2) for |x|<0.5, 10 coefficients')
    array('asin_qS', [mpf(s) for s in [
        '-5.014859407482408326519083440151745519205E3',
        ' 2.430653047950480068881028451580393430537E4',
        '-4.997904737193653607449250593976069726962E4',
        ' 5.675712336110456923807959930107347511086E4',
        '-3.881523118339661268482937768522572588022E4',
        ' 1.634202194895541569749717032234510811216E4',
        '-4.151452662440709301601820849901296953752E3',
        ' 5.956050864057192019085175976175695342168E2',
        '-4.175375777334867025769346564600396877176E1',
    ]], 'asin Q(x^2) for |x|<0.5 (monic), 9 coefficients')

    # asin(0.5625 + t) = asin(0.5625) + t*rS(t)/sS(t),  |t| < 0.0625
    from mpmath import asin as mpasin
    scalar('asinr5625', mpasin(mpf('0.5625')), 'asin(0.5625)')
    array('asin_rS', [mpf(s) for s in [
        '-5.619049346208901520945464704848780243887E0',
        ' 4.460504162777731472539175700169871920352E1',
        '-1.317669505315409261479577040530751477488E2',
        ' 1.626532582423661989632442410808596009227E2',
        '-3.144806644195158614904369445440583873264E1',
        '-9.806674443470740708765165604769099559553E1',
        ' 5.708468492052010816555762842394927806920E1',
        ' 1.396540499232262112248553357962639431922E1',
        '-1.126243289311910363001762058295832610344E1',
        '-4.956179821329901954211277873774472383512E-1',
        ' 3.313227657082367169241333738391762525780E-1',
    ]], 'asin R(t) centered at 0.5625, 11 coefficients')
    array('asin_sS', [mpf(s) for s in [
        '-4.645814742084009935700221277307007679325E0',
        ' 3.879074822457694323970438316317961918430E1',
        '-1.221986588013474694623973554726201001066E2',
        ' 1.658821150347718105012079876756201905822E2',
        '-4.804379630977558197953176474426239748977E1',
        '-1.004296417397316948114344573811562952793E2',
        ' 7.530281592861320234941101403870010111138E1',
        ' 1.270735595411673647119592092304357226607E1',
        '-1.815144839646376500705105967064792930282E1',
    ...
    scalar('lgam_x0', x0, 'lgamma minimum location x0')
    scalar('lgam_y0', log(mpgamma(x0)), 'lgamma(x0) = y0')

    # lgamma reference values — computed from mpmath for full precision
    scalar('lgam1r25', log(mpgamma(mpf('1.25'))), 'lgamma(1.25)')
    scalar('lgam1r75', log(mpgamma(mpf('1.75'))), 'lgamma(1.75)')
    scalar('lgam2r5', log(mpgamma(mpf('2.5'))), 'lgamma(2.5)')
    for n in range(3, 14):
        scalar(f'lgam{n}', log(factorial(n - 1)),
               f'lgamma({n}) = ln({n-1}!)')

    # lgamma(x+1) = x*P(x)/Q(x), 0 <= x <= 0.125 (1.0 <= x+1 <= 1.125)
    array('lgam_RN1', [mpf(s) for s in [
    ...
    # lgamma(x+1) = x*P(x)/Q(x), -0.125 <= x <= 0 (0.875 <= x+1 <= 1.0)
    array('lgam_RNr9', [mpf(s) for s in [
    ...
    array('lgam_RDr9', [mpf(s) for s in [
    ...
    # lgamma(x+1.25) = lgam1r25 + x*P(x)/Q(x), -0.125 <= x <= 0.125
    array('lgam_RN1r25', [mpf(s) for s in [
    ...
    array('lgam_RD1r25', [mpf(s) for s in [
    ...
    # lgamma(x0+z) = y0 + z^2*P(z)/Q(z), near minimum x0~1.4616
    array('lgam_RN1r5', [mpf(s) for s in [
    ...
    array('lgam_RD1r5', [mpf(s) for s in [
    ...
    # lgamma(x+1.75) = lgam1r75 + x*P(x)/Q(x), -0.125 <= x <= 0.125
    array('lgam_RN1r75', [mpf(s) for s in [
    ...
    array('lgam_RD1r75', [mpf(s) for s in [
    ...
    # lgamma(x+2) = x*P(x)/Q(x), -0.125 <= x <= 0.375
    array('lgam_RN2', [mpf(s) for s in [
    ...
    # lgamma(x+2.5) = lgam2r5 + x*P(x)/Q(x), -0.125 <= x <= 0.25
    array('lgam_RN2r5', [mpf(s) for s in [
    ...
    array('lgam_RD2r5', [mpf(s) for s in [
    ...
    # lgamma(x+3) = lgam3 + x*P(x)/Q(x), -0.25 <= x <= 0.5
    array('lgam_RN3', [mpf(s) for s in [
    ...
    # lgamma(x+4) = lgam4 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN4', [mpf(s) for s in [
    ...
    # lgamma(x+5) = lgam5 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN5', [mpf(s) for s in [
    ...
    # lgamma(x+6) = lgam6 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN6', [mpf(s) for s in [
    ...
    # lgamma(x+7) = lgam7 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN7', [mpf(s) for s in [
    ...
    # lgamma(x+8) = lgam8 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN8', [mpf(s) for s in [
    ...
    # lgamma(x+9) = lgam9 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN9', [mpf(s) for s in [
    ...
    # lgamma(x+10) = lgam10 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN10', [mpf(s) for s in [
    ...
    array('lgam_RD10', [mpf(s) for s in [
    ...
    # lgamma(x+11) = lgam11 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN11', [mpf(s) for s in [
    ...
    array('lgam_RD11', [mpf(s) for s in [
    ...
    # lgamma(x+12) = lgam12 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN12', [mpf(s) for s in [
    ...
    array('lgam_RD12', [mpf(s) for s in [
    ...
    # lgamma(x+13) = lgam13 + x*P(x)/Q(x), -0.5 <= x <= 0.5
    array('lgam_RN13', [mpf(s) for s in [
    ...
    array('lgam_RD13', [mpf(s) for s in [
...
def hx(v):
    # Lossless round-trip via IEEE-754 hex float (C++17+ literal syntax).
    return float(v).hex()
...
def write_cpp(groups, f):
    ...
    for g in groups:
        ...
        if kind == 'scalar':
            ...
            f.write(f"inline constexpr double {g['name']}_lo = {hx(g['lo'])};\n\n")
            ...
            f.write(f"inline constexpr double {g['name']}2 = {hx(g['v2'])};\n")
            f.write(f"inline constexpr double {g['name']}3 = {hx(g['v3'])};\n")
            f.write(f"inline constexpr double {g['name']}4 = {hx(g['v4'])};\n\n")

        elif kind == 'exp2_clamp':
            f.write(f"// {g['comment']}\n")
            f.write(f"inline constexpr double exp2_min = {hx(g['min'])};\n")
            f.write(f"inline constexpr double exp2_max = {hx(g['max'])};\n\n")

        elif kind == 'exp_clamp':
            f.write(f"// {g['comment']}\n")
            f.write(f"inline constexpr double exp_max = {hx(g['max'])};\n")
            f.write(f"inline constexpr double exp_min = {hx(g['min'])};\n\n")
            ...
            for i in range(0, n, 2):
                vals = [hx(g['hi'][j]) for j in range(i, min(i+2, n))]
                end = "};" if i + 2 >= n else ","
                f.write(f"    {', '.join(vals)}{end}\n")
            f.write(f"inline constexpr double {g['name']}_lo[{n}] = {{\n")
            for i in range(0, n, 2):
                vals = [hx(g['lo'][j]) for j in range(i, min(i+2, n))]
                end = "};" if i + 2 >= n else ","
                f.write(f"    {', '.join(vals)}{end}\n")
            f.write("\n")
            ...
            for suffix, data in (('_hi', g['hi']), ('_lo', g['mid']),
                ...
                for i in range(0, n, 2):
                    vals = [hx(data[j]) for j in range(i, min(i+2, n))]
                    end = "};" if i + 2 >= n else ","
                    f.write(f"    {', '.join(vals)}{end}\n")
            f.write("\n")
            ...
            for i in range(0, n, 2):
                chunk = [hx(v) for v in vals[i:i+2]]
                end = "};" if i + 2 >= n else ","
